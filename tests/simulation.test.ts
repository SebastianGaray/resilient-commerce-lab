...
describe("deterministic simulation", () => {
  ...
  it("reports the active-client population for each scenario", () => {
    const expected = { low: 50, normal: 250, cyber: 3000, dos: 10000 }
    Object.entries(expected).forEach(([scenario, clients]) => {
      const config = defaultConfig()
      config.scenario = scenario as typeof config.scenario
      expect(simulate(config).activeClients).toBe(clients)
    })
  })
  ...
  it("compares the same seed and traffic inputs", () => {
    ...
    expect(result.baseline.metrics.offered).toBe(result.current.metrics.offered)
