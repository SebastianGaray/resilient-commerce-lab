...
The simulator accepts up to 1,000 arrivals and accepted requests per second, but retains only bounded trace and playback samples. Playback event offsets and half-speed travel durations are normalized into the complete ten-second window. SVG mechanism badges use predefined anchor coordinates for each node or edge. Native popovers provide localized section-level glossaries, the compact legend uses native details/summary, and the activity region reserves five rows while rendering only the latest five events.
