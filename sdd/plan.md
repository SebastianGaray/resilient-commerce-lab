...
The simulator accepts up to 1,000 arrivals and accepted requests per second, but retains only bounded trace and playback samples. Defaults disable optional resilience controls and use a 5,000 ms lab ceiling as the no-timeout-guard state. Playback event offsets and half-speed travel durations are normalized into the complete ten-second window. SVG mechanism badges use predefined anchor coordinates for each node or edge; the circuit breaker targets the dependency path. Native popovers provide localized section-level glossaries, the compact legend uses native details/summary, and the activity region reserves five rows while rendering only the latest five events. Customer output is gated by non-zero virtual playback evidence.
