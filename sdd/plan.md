...
Events use `(time, sequence)` ordering. A documented seeded generator controls latency, errors and jitter. Traffic is bounded per run. Queue capacity and worker service rate create backpressure. Trace capture reserves successful orders so playback visibly represents Queue and Worker without changing their real traffic share. Timeouts race dependencies. retries are bounded and scheduled with exponential backoff plus seeded jitter. Circuit state transitions are explicit. Token-bucket limiting and idempotency records are deterministic.

`capacity.ts` owns scenario arrival curves, educational active-client populations, request-cost weights, per-service capacities, CPU/memory estimates and scaling. Horizontal desired replicas use `ceil(demand / (perInstanceCapacity × 0.70))` capped by the selected 2, 4 or 8 pod plan. Vertical scaling keeps one instance and doubles capacity. Error probability grows only above 85% modeled utilization, while a quadratic latency penalty begins above 70%. These are transparent educational heuristics, not capacity recommendations. The UI reports capacity units, not currency.
...
The simulator accepts up to 1,000 arrivals and accepted requests per second, but retains only bounded trace and playback samples. Defaults disable optional resilience controls and use a 5,000 ms lab ceiling as the no-timeout-guard state. Each simulated second stores a resource snapshot that playback reveals by progress. Playback event offsets and half-speed travel durations are normalized into the complete ten-second window. SVG mechanism badges use predefined anchor coordinates for each node or edge. The circuit breaker targets the dependency path. Native popovers provide localized section-level glossaries, the compact legend uses native details/summary, and the activity region reserves five rows while rendering only the latest five events. Customer output and a bounded recommendation list are gated by non-zero virtual playback evidence.
