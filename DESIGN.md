...
The topology is a projection of simulator state, not a second editor. Cache and limiter nodes appear only when their existing controls require them. resilience mechanisms become annotations. Circuit breaker is anchored on the dependency-call path, away from Cache. Capacity remains a textual resource grid instead of adding infrastructure nodes. Play/Pause is one stateful action, a single run occupies the complete ten-second window, and continuous mode loops that same seeded run. A collapsed legend groups the detailed orb outcomes into four readable categories and sits directly below the diagram, before service capacity.
...
Traffic uses four named scenarios rather than independent rate, pattern or fault controls. Scaling is a separate choice. Resilience controls follow the request path in four numbered fieldsets. Help popovers disclose the educational utilization formulas and link their official Kubernetes, Google SRE and AWS sources.

Operational metrics use monospace values and plain-language labels. Traces use a vertical timing list on narrow screens. Service capacity is empty before Play and advances from the per-second simulation timeline. The customer preview and recommendations remain absent until playback produces customer evidence. Guidance then lists several applicable actions with a reason for each. The default configuration intentionally enables no optional resilience mechanism. One scaling selector exposes no scaling, horizontal 2/4/8-pod caps and vertical 2× capacity. The relative footprint avoids pretending to estimate currency. Panels reflow without horizontal page scroll at the supported 20rem minimum.

Traffic labels state their educational active-client populations. The fixed deterministic seed is internal rather than user-configurable. Client copy and documentation avoid semicolons. JavaScript and TypeScript formatting omits statement terminators while syntax-required CSS and protocol separators remain unchanged.
