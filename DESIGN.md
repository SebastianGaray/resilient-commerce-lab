...
The topology is a projection of simulator state, not a second editor. Cache and limiter nodes appear only when their existing controls require them; resilience mechanisms become annotations. Capacity remains a textual resource grid instead of adding infrastructure nodes. Play/Pause is one stateful action, a single run occupies the complete ten-second window, and continuous mode loops that same seeded run. A collapsed legend groups the detailed orb outcomes into four readable categories.
...
Operational metrics use monospace values and plain-language labels. Traces use a vertical timing list on narrow screens. The customer preview remains subordinate to the simulator, always carries a simulated label and offers one configuration-aware next step. Horizontal scaling exposes a per-service maximum and a relative capacity footprint rather than pretending to estimate currency. Panels reflow without horizontal page scroll at the supported 20rem minimum.
